...
{
	int time = mainClock * 1000;
	TileType type = mapita[i][j];
	vec pos = vec(i * TILE_SIZE, j * TILE_SIZE);

	switch (type) {
	case TileType::ROOMBA_HOME:
	{
		Window::Draw(Assets::sprite, pos).withRect(0, 13 * TILE_SIZE, TILE_SIZE, TILE_SIZE);
	} break;
	case TileType::SIGN_GOOD:
		Window::Draw(Assets::sprite, pos).withRect(5*16, 10 * 16, 16, 16);
		break;
	case TileType::SIGN_BAD:
		Window::Draw(Assets::sprite, pos).withRect(6 * 16, 10 * 16, 16, 16);
		break;
	case TileType::FLOOR:
		Window::Draw(Assets::sprite, pos).withRect(64, 48, 16, 16);
		break;
	case TileType::WALL:
	{
		Window::Draw(Assets::sprite, pos).withRect(64 + 16, 48, 16, 16);
	} break;
	case TileType::BELT_DOWN:
		Window::Draw(Assets::sprite, pos).withRect(Animation::AnimFrame(AnimationType::BELT_RIGHT, time)).withOrigin(0, 16).withRotationDegs(90);
		break;
	case TileType::BELT_UP:
		Window::Draw(Assets::sprite, pos).withRect(Animation::AnimFrame(AnimationType::BELT_RIGHT, time)).withOrigin(16, 0).withRotationDegs(-90);
		break;
	case TileType::BELT_LEFT:
		Window::Draw(Assets::sprite, pos).withRect(Animation::AnimFrame(AnimationType::BELT_RIGHT, time)).withOrigin(16, 16).withRotationDegs(180);
		break;
	case TileType::BELT_RIGHT:
		Window::Draw(Assets::sprite, pos).withRect(Animation::AnimFrame(AnimationType::BELT_RIGHT, time));
		break;
	}
}
