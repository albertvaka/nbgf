...
void Input::MapGameKeysToGamePad() {
	gp_map[(int)GameKeys::UP] = [](int p) { // p is the player id
	...
	gp_map[(int)GameKeys::DOWN] = [](int p) {
	...
	gp_map[(int)GameKeys::ACTION] = [](int p) { return GamePad::IsButtonPressed(p, SDL_CONTROLLER_BUTTON_A); };
	gp_map[(int)GameKeys::START] = [](int p) { return GamePad::IsButtonPressed(p, SDL_CONTROLLER_BUTTON_START); };
}

std::function<bool()> Input::kb_map[magic_enum::enum_count<GameKeys>()];
void Input::MapGameKeysToKeyboard()
{
	kb_map[(int)GameKeys::UP] = []() {
		return Keyboard::IsKeyPressed(SDL_SCANCODE_W) || Keyboard::IsKeyPressed(SDL_SCANCODE_UP);
	};
	kb_map[(int)GameKeys::DOWN] = []() {
