...
const BoxBounds& Stroke::GetBoundingBox()
{
...
void Stroke::DrawSolidBlocExcluding(const Stroke& other, unsigned int current_joint, unsigned int previous_joint) const
{
	...
	vec other_upper_previous = other.joints[previous_joint].upper_point + other.joints[previous_joint].offset;
	vec other_upper_next = other.joints[current_joint].upper_point + other.joints[current_joint].offset;
	vec other_lower_previous = other.joints[previous_joint].lower_point + other.joints[previous_joint].offset;
	vec other_lower_next = other.joints[current_joint].lower_point + other.joints[current_joint].offset;
	...
	{
		{
			...
			if (Mates::Sign(angle1) != Mates::Sign(angle2) && ((angle1 > 90 || angle1 < -90) || (angle2 > 90 || angle2 < -90)) && (Mates::Norm(angle1) + Mates::Norm(angle2) > 180))
			{
				if (joints[0].angle >= 0)
					joints[0].angle -= 180;
				else
