...
int AddEntity(EntityType type, float x, float y, int player = -1, float rot = 0, EntityType parent_type = EntityType::NONE)
{
...
void UpdateShot(float dt, int id)
{
...
void EntityRotateTo(int id, float dt, float rot)
{
	while (rot > 360.f) {
		rot -= 360.f;
	}
...
void UpdatePlayer(float dt, int id)
{
	...
	if (gd.ent_collision_matrix[id][gd.id_hq[gd.ent_player[id]]])
	{
		...
		if (gd.ent_carry[id] == EntityType::MINERAL)
		{
			...
			AddEntity(EntityType::MINERAL_GAIN, gd.ent_x[id], gd.ent_y[id], num_player);
		...
		if (Input::IsJustPressed(num_player, GameKeys::ACCEPT))
		{
			if (gd.money[num_player] >= GameConstants::PRICE_WORKER)
			{
				...
				int id_worker = AddEntity(EntityType::WORKER, hq_x, hq_y, num_player);
			}
		}
		...
		if (Input::IsJustPressed(num_player, GameKeys::CANCEL))
		{
			if (gd.money[num_player] >= GameConstants::PRICE_SOLDIER)
			{
				...
				int id_soldier = AddEntity(EntityType::SOLDIER, hq_x, hq_y, num_player);
	...
	if (gd.ent_cooldown_shoot[id] < 0.f) {
		...
		if (GamePad::Trigger::Right.IsPressed(num_player))
		{
			...
			AddEntity(EntityType::SHOT, gd.ent_x[id] + DISTANSITA_CANYONES * dir_x, gd.ent_y[id] + DISTANSITA_CANYONES * dir_y, num_player, gd.ent_rot[id] - ANGULITO_JORDI_PUJOL, EntityType::PLAYER);
			AddEntity(EntityType::SHOT, gd.ent_x[id] - DISTANSITA_CANYONES * dir_x, gd.ent_y[id] - DISTANSITA_CANYONES * dir_y, num_player, gd.ent_rot[id] + ANGULITO_JORDI_PUJOL, EntityType::PLAYER);
		}
...
void UpdateWorker(float dt, int id)
{
...
void UpdateSoldier(float dt, int id)
{
	...
	if (id_target > -1) {
		...
		if (dir_target.Length() <= GameConstants::SOLDIER_ATTACK_RADIUS)
		{
			...
			EntityRotateTo(id, dt, rot_target);
		}
		else {
			id_target = -1;
		}
	...
	if (gd.ent_cooldown_shoot[id] < 0.0f)
	{
		...
		if (id_nearest != -1 && dist_nearest <= GameConstants::SOLDIER_ATTACK_RADIUS + 10)
		{
			...
			int shotecito = AddEntity(EntityType::SHOT, pos.x, pos.y, num_player, rot_shoot, EntityType::SOLDIER);
			gd.ent_cooldown_shoot[id] = 1.0f;
			EntityRotateTo(shotecito, dt, rot_shoot);
...
bool IsEntityCollision(int e1, int e2)
{
...
void UpdateHQ(float dt, int id)
{
	...
	if (gd.ent_cooldown_shoot[id] > 0.f) {
		...
		if (target > -1) {
			...
			int shotecito = AddEntity(EntityType::SHOT, pos.x, pos.y, player, Angles::RadsToDegs(atan2(gd.ent_y[target] - pos.y, gd.ent_x[target] - pos.x)) + 90, EntityType::HQ);
...
void UpdateEntities(float dt)
{
	...
	memset(gd.ent_collision_matrix, 0, sizeof(gd.ent_collision_matrix));
	for (int i = 0; i < GameConstants::MAX_ENTITIES; ++i)
	{
		int t1 = gd.ent_type[i];
		if (t1 == EntityType::NONE) continue;

		//Collisions
		for (int j = i + 1; j < GameConstants::MAX_ENTITIES; ++j)
		{
			int t2 = gd.ent_type[j];
			if (t2 == EntityType::NONE) continue;
		...
		if (gd.ent_hp[i] <= 0 && gd.ent_type[i] != EntityType::NONE)
		{
			...
			{
				AddEntity(EntityType::EXPLOSION_MINION, gd.ent_x[i], gd.ent_y[i]);
			}
	...
	for (int p = 0; p < 2; p++) {
		if (gd.player_respawn_time[p] > 0.f) {
			...
			if (gd.player_respawn_time[p] <= 0.f) {
				...
				gd.id_player[p] = AddEntity(EntityType::PLAYER, x, Window::GAME_HEIGHT / 2, p, r);
			}
		}
	}
...
void InitGame()
{
	...
	gd.id_mineral[0][0] = AddEntity(EntityType::MINERAL, 140, 250, 0);
	gd.id_mineral[0][1] = AddEntity(EntityType::MINERAL, 140, 850, 0);

	gd.id_mineral[1][0] = AddEntity(EntityType::MINERAL, Window::GAME_WIDTH - 140, 250, 1);
	gd.id_mineral[1][1] = AddEntity(EntityType::MINERAL, Window::GAME_WIDTH - 140, 850, 1);

	gd.id_hq[0] = AddEntity(EntityType::HQ, 250, 530, 0);
	gd.id_hq[1] = AddEntity(EntityType::HQ, Window::GAME_WIDTH - 250, 530, 1);

	gd.id_player[0] = AddEntity(EntityType::PLAYER, 400, Window::GAME_HEIGHT / 2, 0, 90);
	gd.id_player[1] = AddEntity(EntityType::PLAYER, Window::GAME_WIDTH - 400, Window::GAME_HEIGHT / 2, 1, 270);
...
void RenderHealthBar(int i) {
	if (gd.ent_hp[i] < 1000)
	{
		...
		Window::DrawPrimitive::Rectangle(b, 1.5f, { 255,255,255,255 });
...
void RenderEntities() {
	...
	for (int i = 0; i < GameConstants::MAX_ENTITIES; ++i)
	{
		...
		if (gd.ent_type[i] == EntityType::EXPLOSION_MINION)
		{
			...
			sprite = GetSpriteForType(gd.ent_type[i], gd.ent_player[i]);
		}
		...
		Window::Draw(sprite, vec(gd.ent_x[i], gd.ent_y[i]))
			.withOriginAtSpriteCenter()
