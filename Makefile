...
ifdef EMSCRIPTEN
	...
	PLATFORM_CFLAGS=-Oz -DSDL_GPU_DISABLE_OPENGL -DSDL_GPU_DISABLE_GLES_1 -s USE_SDL_TTF=2 -s USE_SDL_MIXER=2 -s USE_OGG -s USE_VORBIS -s ALLOW_MEMORY_GROWTH=1 --preload-file bin/data@/data --use-preload-plugins $(WEBGL_CFLAGS)
	PLATFORM_LDFLAGS=
else
	OUT_FILE=$(EXEC)
