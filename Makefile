...
ifdef EMSCRIPTEN
	...
	PLATFORM_CFLAGS=-Oz -s USE_SDL_TTF=2 -s USE_SDL_MIXER=2 -s USE_OGG -s USE_VORBIS -s ALLOW_MEMORY_GROWTH=1 --preload-file bin/data@/data --use-preload-plugins $(WEBGL_FLAGS)
	PLATFORM_LDFLAGS=
else
	OUT_FILE=$(EXEC)
	ifeq ($(shell uname),Linux)
		...
		# MacOS
		PLATFORM_CFLAGS=-DSDL_GPU_DISABLE_OPENGL_4
