...
struct AnimationFrame
{
	...
	constexpr float GetDuration() const { return duration; }
};
...
struct SheetFrameCalculator {
	...
	const int columns;
	const float time;
	consteval SheetFrameCalculator(float sprite_w, float sprite_h, int columns, float time)
		: sprite_w(sprite_w)
		, sprite_h(sprite_h)
		, columns(columns)
		, time(time)
	...
	constexpr const AnimationFrame Frame(int index, float t = -1.f) const {
